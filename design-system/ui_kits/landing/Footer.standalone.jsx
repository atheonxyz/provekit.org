// Footer.standalone.jsx — uses window.__resources for wordmarks; no benchmarks page link
...
function AsciiWordmark() {
  ...
  React.useEffect(() => {
    ...
    img.src = window.__resources["wordmark-large"];
  }, []);

  React.useEffect(() => {
    if (!grid || !canvasRef.current) return;
    const canvas = canvasRef.current;
    const cellW = 16, cellH = 28;
    ...
    const draw = () => {
      ...
      ctx.font = `800 ${cellH - 4}px ui-monospace, "JetBrains Mono", monospace`;
      ctx.textBaseline = "top";
      for (let y = 0; y < grid.rows; y++) {
        for (let x = 0; x < grid.cols; x++) {
          const d = grid.cells[y * grid.cols + x];
          if (d < 0.02) continue;
          const dBoost = Math.pow(d, 0.26);
          let mod = 1;
          if (hover) {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <img src={window.__resources["wordmark-large-blue"]} alt="Provekit"
...
function Footer() {
  const onBench = typeof window !== "undefined" && /Provekit Benchmarks\.html$/i.test(decodeURIComponent(window.location.pathname));
  return (
    <footer style={{ width: 1600, maxWidth: "100%", padding: "48px 24px 24px", display: "flex", flexDirection: "column", gap: 12, position: "relative", overflow: "hidden", borderLeft: "1px solid var(--pk-line)", borderRight: "1px solid var(--pk-line)", boxSizing: "border-box" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 8 }}>
          <Chip>DOCS</Chip>
          <Chip>GUIDE</Chip>
          <Chip href={onBench ? "Provekit Landing.html" : "Provekit Benchmarks.html"}>{onBench ? "HOME" : "BENCHMARKS"}</Chip>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <Chip>TELEGRAM</Chip>
          <Chip>TWITTER</Chip>
